...
7. Optional: `set_redirects` with `/*    /index.html   200` for History-API SPAs (then `deploy`), `rollback` (Pro/Scale, last 5 successful deploys), `configure_custom_domain`, `create_form`, `get_quotas_and_usage`, `update_project_settings` (`analytics_enabled: true`) then `get_project_analytics`, `update_project_settings` (`share_gate_emails`) for optional email OTP on `*.novence.ai` only (custom domains stay public), `invite_project_member` (Pro/Scale owner). Upload `/404.html` for a custom 404 page.
